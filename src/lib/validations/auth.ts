  ...
  .refine(
    (data) => {
      if (data.role === 'teacher') {
        return !!data.phone && /^\+?[0-9\s-]{7,15}$/.test(data.phone.trim());
      }
      if (data.phone && data.phone.trim().length > 0) {
        return /^\+?[0-9\s-]{7,15}$/.test(data.phone.trim());
      }
      return true;
    },
    {
      message: 'Müəllim hesabı üçün düzgün telefon nömrəsi tələb olunur',
      path: ['phone'],
    }
  );
