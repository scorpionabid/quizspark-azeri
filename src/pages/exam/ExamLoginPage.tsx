...
export default function ExamLoginPage() {
  ...
  const isFormValid = isFinValid && isNameValid && !!selectedKey && agreed;
        ...
        {completedInfo ? (
          ...
          <Card className="border-slate-200 dark:border-slate-800 shadow-xl bg-white dark:bg-slate-900">
            <CardHeader className="pb-4">
              ...
              <CardDescription>
                İmtahana daxil olmaq üçün şəxsiyyət vəsiqənizin FİN kodunu, ad və soyadınızı yazın və imtahan ixtisasınızı seçin.
              </CardDescription>
            </CardHeader>
              ...
              <form onSubmit={handleStartExam} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between">
                    <Label htmlFor="fin" className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                      FİN Kod (Şəxsiyyət Vəsiqəsi) <span className="text-red-500">*</span>
                    </Label>
                    <span className="text-xs text-slate-400">{cleanFin.length} / 7 simvol</span>
                  </div>
                  <div className="relative">
                    <Input
                      ...
                      maxLength={7}
                      className="font-mono text-base tracking-wider uppercase font-semibold h-11 pl-3 pr-10"
                      autoComplete="off"
                      required
                    />
                    {isFinValid && (
                      <CheckCircle2 className="w-5 h-5 text-emerald-500 absolute right-3 top-3 pointer-events-none" />
                    )}
                  </div>
                  <p className="text-[11px] text-slate-500">
                    Yeni nəsil şəxsiyyət vəsiqələrində aşağı sağ küncdəki 7 simvol, köhnələrdə seriya nömrəsinin son 7 simvolu.
                  </p>
                </div>
                ...
                {/* Specialty Select */}
                <div className="space-y-1.5">
                  <Label htmlFor="specialty" className="text-sm font-semibold text-slate-800 dark:text-slate-200">
                    İmtahan İxtisası / Vakansiya <span className="text-red-500">*</span>
                  </Label>
                  <Select value={selectedKey} onValueChange={setSelectedKey}>
                    <SelectTrigger id="specialty" className="h-11 text-sm">
                      <SelectValue placeholder="İmtahan veriləcək ixtisası seçin..." />
                    </SelectTrigger>
                    <SelectContent>
                      {OFFICIAL_EXAMS.map((exam) => (
                        <SelectItem key={exam.key} value={exam.key} className="py-2">
                ...
                <div className="p-3.5 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200 dark:border-amber-800 text-amber-900 dark:text-amber-200 text-xs space-y-1.5">
                  <div className="font-bold flex items-center gap-1.5">
                    <AlertCircle className="w-4 h-4 text-amber-600" />
                    Mühüm İmtahan Qaydaları:
                  </div>
                  <ul className="list-disc list-inside space-y-1 pl-1 text-[11px] leading-relaxed">
                    <li>İmtahan müddəti 120 dəqiqədir (2 saat). Vaxt bitdikdə sistem avtomatik yekunlaşır.</li>
                    <li>Hər iştirakçıya yalnız <strong>1 cəhd</strong> hüququ verilir.</li>
                    <li>Səhifəni yeniləsəniz və ya internet qopsa belə, qeyd etdiyiniz cavablar itmir.</li>
                    <li>İmtahan zamanı brauzer pəncərəsini tərk etmək qayda pozuntusu hesab olunur.</li>
                  </ul>
                </div>
                ...
                <div className="flex items-start space-x-2.5 pt-1">
                  ...
                  >
                    Yuxarıdakı imtahan qaydaları və şərtləri ilə tanış oldum, daxil etdiyim FİN kodun və məlumatların doğruluğunu təsdiq edirəm.
                  </Label>
                </div>
                ...
                >
                  {isLoading ? (
                    ...
                    <span className="flex items-center gap-2">
                      İmtahana Başla <ArrowRight className="w-5 h-5" />
                    </span>
                  )}
                </Button>
              </form>
            ...
            <CardFooter className="border-t border-slate-100 dark:border-slate-800 pt-4 flex justify-between text-xs text-slate-500">
